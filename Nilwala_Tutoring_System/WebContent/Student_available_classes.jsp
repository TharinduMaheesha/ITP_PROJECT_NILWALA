...
<div>
	...
	<input type="text" id="tm_myInput" onkeyup="myFunction()" placeholder="Search for subject name.." title="Type in a name">
			...
			ArrayList<SubjectClass> array = ClassControllerUtil.getStudentSubjects(id);
		...
		<p style = "font-family: sans-serif; font-size: 20px ; text-align: center;"><b> Enroll to attend a class </b></p>
			...
			<br><br>
				<table id="tm_myTable">
				  ...
				  <%	for(SubjectClass subject  : array){
					  
					  String tid = subject.getTid();
					  String code = subject.getSubject();
					  if(tid != null && code != null){
						  
					  ArrayList<SubjectClass> array2 = ClassControllerUtil.StudentAvailableClasses(tid, code);
					  for(SubjectClass sub  : array2){

					  
					 	 String status = ClassControllerUtil.checkEnrolled(sub.getId(), uid);
   ...
   <form  action="StudentClassControllerServlet" method="post">
				  ...
				  <tr>
				    ...
				    <td><b><%=sub.getStart() %></b>	</td>
				       ...
				       <td><button name = "StudentClass" value = "Enroll" style="width:auto;">Enroll</button>
	...
	%>
     ...
     <input type="submit" value="View Enrolled Classes" name = "StudentClass"style = "margin-left : 100px; font-size:20px; width : 300px ;color : white; box-shadow: 0 8px 16px 0 rgba(0,0,0,0.2), 0 6px 20px 0 rgba(0,0,0,0.19); padding: 20px 24px ; background-color: #3000b3"></form>
    </div>
				<script>
						function myFunction() {
